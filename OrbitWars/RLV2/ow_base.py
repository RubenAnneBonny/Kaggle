...
# ===== v2 additions: comet-aware pathing + aggressive BC teacher ===========
#
# WHY: in v1 the network's ship count was bypassed by capture_size, which
# routed every attack through how_many_send (whose path_blocked check already
# avoids planets AND the orbiting-comet approximation). In v2 the network owns
# the ship count, so launches go through reach() — which only avoids the SUN.
# That would let fleets fly straight through comets and get eaten. These helpers
# add a faithful comet/planet path check that uses the comet's ACTUAL path data
# (obs["comets"]), not the (wrong) orbital-rotation approximation.

def comet_path_map(obs):
    """pid -> (path_list, path_index) for every live comet, from obs['comets'].
    Lets us compute a comet's exact position k ticks from now: path[index + k]."""
    groups = get_field(obs, "comets", []) or []
    cmap = {}
    for g in groups:
        idx = g.get("path_index", -1) if isinstance(g, dict) else getattr(g, "path_index", -1)
        pids = g["planet_ids"] if isinstance(g, dict) else g.planet_ids
        paths = g["paths"] if isinstance(g, dict) else g.paths
        for i, pid in enumerate(pids):
            cmap[pid] = (paths[i], idx)
    return cmap


def _comet_pos(cmap, pid, k):
    """Comet pid's position k ticks in the future (k=0 == now). None if expired."""
    path, idx = cmap[pid]
    j = idx + k
...
def path_clear(src, angle, ships, t_arrive, obs, target_id, comet_buffer=1.0):
    """True if a fleet of `ships` launched from `src` at `angle` reaches its
    target WITHOUT sweeping any non-target planet or any comet first.

    Comets are checked against their real future path positions; ordinary
    planets against their rotation (matching the engine). This is what keeps the
    agent from 'accidentally shooting' a comet once the network controls size."""
    ang_vel = get_field(obs, "angular_velocity", 0)
    planets = [Planet(*p) for p in get_field(obs, "planets", [])]
    cmap = comet_path_map(obs)
    comet_ids = set(cmap.keys())
    ...
    others = [p for p in planets if p.id != target_id and p.id != src.id]
    last = min(int(math.ceil(t_arrive)), 100)
    for t in range(1, last + 1):
        nfx, nfy = fx + dx, fy + dy
        for p in others:
            r = p.radius + (comet_buffer if p.id in comet_ids else 0.0)
            if p.id in comet_ids:
                o = _comet_pos(cmap, p.id, t - 1)
                n = _comet_pos(cmap, p.id, t)
                ...
                if o is None:
                    o = n
                p0x, p0y, p1x, p1y = o[0], o[1], n[0], n[1]
            elif is_orbiting(p):
                a0 = (t - 1) * ang_vel
                a1 = t * ang_vel
                ddx, ddy = p.x - CENTER, p.y - CENTER
                p0x = CENTER + math.cos(a0) * ddx - math.sin(a0) * ddy
                p0y = CENTER + math.sin(a0) * ddx + math.cos(a0) * ddy
                p1x = CENTER + math.cos(a1) * ddx - math.sin(a1) * ddy
                p1y = CENTER + math.sin(a1) * ddx + math.cos(a1) * ddy
            else:
                p0x = p1x = p.x
                p0y = p1y = p.y
            if _swept_hit(fx, fy, nfx, nfy, p0x, p0y, p1x, p1y, r):
                return False
        fx, fy = nfx, nfy
    return True


def reach_safe(src, dst, ships, obs):
    """Like reach() but also rejects paths that would sweep a planet or comet.
    Returns (t_star, angle) or None (unreachable, sun-blocked, or path blocked)."""
    sol = reach(src, dst, ships, obs)
    if sol is None:
        return None
    t_star, angle = sol
    if not path_clear(src, angle, ships, t_star, obs, dst.id):
        return None
    return sol


AGGR_EXTRA_SPARE = 14        # bigger safety margin -> bigger clear-path fleets
AGGR_MIN_ATTACK_FRAC = 0.55  # once attacking, send at least this frac of available


def net_roi_aggressive(obs):
    """Aggressive variant of net_roi_support, used as the BC TEACHER in v2.

    NOTE: single-arg signature on purpose — kaggle_environments fills positional
    parameters by count, so extra params would bind to the framework's
    configuration/env objects. Tunables live in the AGGR_* module constants.

    Same target selection (defense first, then ROI-ranked attacks), but it
    commits noticeably MORE ships per attack so behavioral cloning starts the
    fraction head high. Two levers, both keeping a verified clear path:
      * extra_spare: a larger safety margin handed to how_many_send_improved, so
        the (still clear-path, still affordable) capturing fleet it returns is
        bigger.
      * min_attack_frac: a floor — once a target is chosen, send at least this
        fraction of *available* ships (re-pathed safely), unless unaffordable.
    It still respects the defensive reserve, so it never strands a planet.
    """
    moves = []
    player, mine, targets, comet_ids, _ = parse_obs(obs)
    if not mine:
    ...
    # ---- Pass 1: defense (identical to net_roi_support) ----
    for tp in sorted((p for p in mine if friendly_net(p) < 0), key=lambda p: etick.get(p.id, 1e9)):
        deficit = -friendly_net(tp) + 1
        for h in sorted((h for h in mine if h.id != tp.id), key=lambda h: sq_dist(h, tp)):
            if deficit <= 0:
    ...
    # ---- Pass 2: attack, ranked by ROI, sized AGGRESSIVELY ----
    def value(planet, p):
        d = math.sqrt(sq_dist(planet, p))
        return p.production / (eff_garrison(p) + 1) - 0.02 * d
    ...
    for planet in mine:
        ...
        eff = eff_garrison(best)
        adj = best._replace(ships=int(eff))
        # capturing fleet with a fat margin (clear path guaranteed by how_many_send)
        ships, angle, _t = how_many_send_improved(planet, adj, hold_buffer(planet) + AGGR_EXTRA_SPARE, obs)
        if ships is None or ships > a:
            # fat margin unaffordable -> fall back to a plain capturing fleet
            ships, angle, _t = how_many_send_improved(planet, adj, hold_buffer(planet), obs)
            if ships is None or ships > a:
                continue
        # apply the fraction floor: bump the send up and re-path it safely
        floor = int(math.ceil(AGGR_MIN_ATTACK_FRAC * a))
        if floor > ships:
            sol = reach_safe(planet, best, min(floor, a), obs)
            if sol is not None:
                _t2, angle = sol
                ships = min(floor, a)
        moves.append([planet.id, angle, int(ships)])
        attack_committed[best.id] = attack_committed.get(best.id, 0) + int(ships)
        outgoing[planet.id] = outgoing.get(planet.id, 0) + int(ships)
